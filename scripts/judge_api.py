...
# -*- coding: utf-8 -*-
"""
judge_api.py — Claude API 폴백 경로 (구독/claude-code-action이 없을 때 사용)

candidates.json의 각 후보를 Claude(Haiku)로 판정해 agents.csv 스키마의 1행으로
...
환경변수: ANTHROPIC_API_KEY (필수), MODEL (선택, 기본 claude-haiku-4-5)
...
def main():
    ...
    try:
        ...
        print("[error] anthropic 패키지 필요: pip install anthropic", file=sys.stderr)
        sys.exit(1)
